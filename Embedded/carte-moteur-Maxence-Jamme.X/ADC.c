...
AD1CON2bits.SMPI = 4; // 4+1 conversions successives avant interrupt  ici => AD1CON2bits.SMPI = 4;
AD1CON2bits.ALTS = 0;
AD1CON2bits.BUFM = 0;
...
ANSELCbits.ANSC11 = 1;
ANSELGbits.ANSG9 = 1;
ANSELBbits.ANSB1 = 1;   // ici
ANSELEbits.ANSE15 = 1;   // ici

AD1CSSLbits.CSS6=1; // Enable AN6 for scan
AD1CSSLbits.CSS11=1; // Enable AN11 for scan
AD1CSSHbits.CSS16=1; // Enable AN16 for scan
AD1CSSLbits.CSS3=1; // Enable AN3 for scan      // ici
AD1CSSLbits.CSS15=1; // Enable AN15 for scan     // ici
...
void ADC1StartConversionSequence()  //echantillonage lance lacquisition de données
{
AD1CON1bits.SAMP = 1 ; //Lance une acquisition ADC
}

unsigned int * ADCGetResult(void)//Recuperation du resultat
{
return ADCResult;
}

unsigned char ADCIsConversionFinished(void)// verifie le flag si cest a 1 la donnée est a récupéré
{
return ADCConversionFinishedFlag;
}

void ADCClearConversionFinishedFlag(void)//reset flag quand la donnée est récupéré
{
